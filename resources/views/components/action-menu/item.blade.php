{{-- Satu baris aksi di dalam <x-action-menu>. Beri `href` untuk link. --}}
@props([
    'icon' => null,
    'variant' => 'neutral',
    'href' => null,
...
@php
    // Warna teks Ant-flavoured; latar tint tipis saat hover.
    $variants = [
        'neutral' => 'text-gray-700 hover:bg-gray-50',
        'primary' => 'text-[#1677ff] hover:bg-[#e6f4ff]',
        'purple' => 'text-[#722ed1] hover:bg-[#f9f0ff]',
        'warning' => 'text-[#d48806] hover:bg-[#fff7e6]',
        'danger' => 'text-[#cf1322] hover:bg-[#fff1f0]',
        'cyan' => 'text-[#08979c] hover:bg-[#e6fffb]',
        'green' => 'text-[#389e0d] hover:bg-[#f6ffed]',
    ];

    // Heroicons (outline) path data — superset dari x-action-btn.
    $icons = [
    ...
    $cls = 'w-full flex items-center gap-2.5 px-3.5 py-2 text-[13px] font-medium text-left whitespace-nowrap transition-colors '
...
@endif
        @if ($path)
            <svg class="w-4 h-4 shrink-0 opacity-80" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
