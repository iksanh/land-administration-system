<div class="flex flex-col gap-6">
    ...
    @if ($showPrint && $printBa)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" wire:key="ba-print-modal"
            x-data="{
                cetak() {
                    const frame = document.getElementById('berita-acara-print-frame');
                    frame.onload = () => { frame.contentWindow.focus(); frame.contentWindow.print(); };
                    frame.src = '{{ route('berita-acara.print', $printBa->id) }}?t=' + Date.now();
                }
            }">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200">
                    <h3 class="font-semibold text-gray-800">Pratinjau Berita Acara Pemeriksaan Lapang</h3>
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('berita-acara.word', $printBa->id) }}"
                ...
                <div class="overflow-y-auto p-6 bg-gray-100">
                    <div class="bg-white shadow-sm mx-auto p-8" style="max-width:21cm;">
                        @include('berita-acara._dokumen', ['ba' => $printBa, 'mode' => 'print'])
                    </div>
                </div>
            </div>
            {{-- Off-screen (bukan display:none — iframe tersembunyi tak tercetak di sebagian browser). --}}
            <iframe id="berita-acara-print-frame" aria-hidden="true" tabindex="-1"
